...
class SigmaRule:
    ...
    Conditions may reference multiple selections: "sel1 and (sel2 or not sel3)"
    """
...
def generate_synthetic_workspace(base_dir: str, count: int = 100, include_yara: bool = True, include_sigma: bool = True):
    """Create synthetic logs (JSONL), a sigma rules file and optional yara. Useful for load/fuzz testing."""
    logs_dir = os.path.join(base_dir, 'logs')
    rules_dir = os.path.join(base_dir, 'rules')
    yara_dir = os.path.join(base_dir, 'yara')
    os.makedirs(logs_dir, exist_ok=True)
    ...
    with open(path, 'w', encoding='utf-8') as fh:
        for _ in range(count):
            ...
            if t == 1:
                ...
                # arbitrary message possibly containing signature
                msg = 'normal message'
                if randrange(20) == 0:
